...
class SessionSecurity {
    ...
    private static function validateSession() {
        ...
        // IP değişim kontrolü (GEÇİCİ OLARAK DEVRE DIŞI - DEBUG İÇİN)
        if (isset($_SESSION['_original_ip']) && $_SESSION['_original_ip'] !== $ipAddress) {
            error_log("SECURITY WARNING: IP change detected. Original: {$_SESSION['_original_ip']}, Current: $ipAddress - ALLOWING FOR DEBUG");
            // Geçici olarak session'ı yok etmiyoruz
            // self::destroySession();
            // http_response_code(403);
            // die(self::getSecurityViolationPage());
        }
        
        // User Agent değişim kontrolü (GEÇİCİ OLARAK DEVRE DIŞI - DEBUG İÇİN)
        if (isset($_SESSION['_original_user_agent']) && $_SESSION['_original_user_agent'] !== $userAgent) {
            error_log("SECURITY WARNING: User Agent change detected - ALLOWING FOR DEBUG");
            // Geçici olarak session'ı yok etmiyoruz
            // self::destroySession();
            // http_response_code(403);
            // die(self::getSecurityViolationPage());
        ...
        // Core fingerprint kontrolü (GEÇİCİ OLARAK DEVRE DIŞI - DEBUG İÇİN)
        if ($_SESSION['_security_fingerprint'] !== $coreFingerprint) {
            error_log("SECURITY WARNING: Critical device fingerprint change detected - ALLOWING FOR DEBUG");
            // Geçici olarak session'ı yok etmiyoruz
            // self::destroySession();
            // http_response_code(403);
            // die(self::getSecurityViolationPage());
